...
contract("ChainList", function(accounts){
  ...
  it("should throw an exception if you try to buy your own article", function() {
    ...
    .catch(function(error) {
      ...
      assert.equal(data[1], seller, "seller must be " + seller);
      assert.equal(data[2], 0x0, "buyer must be empty");
      assert.equal(data[3], articleName, "article name must be empty" + articleName);
      assert.equal(data[4], articleDescription, "article description must be " + articleDescription);
      assert.equal(data[5].toNumber(), web3.toWei(articlePrice, "ether"), "article price must be " + web3.toWei(articlePrice, "ether"));
    });
  });
  ...
  // incorrect value
  it("should throw an exception if you try to buy an article different from its price", function() {
    return ChainList.deployed().then(function(instance) {
      chainListInstance = instance;
  ...
  it("should throw an exception if you try to buy an article that has already sold", function() {
    return ChainList.deployed().then(function(instance) {
      chainListInstance
      = instance;
